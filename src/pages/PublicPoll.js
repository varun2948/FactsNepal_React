    ...
    render() {
        ...
        return (
            <React.Fragment>
                ...
                {polloftheday &&
                    <div className="banner-content banner-height bg-grey pdb-100">
                        <div className="container">
                            <div className="publicpoll-wrap">
                                <div className="wrap-center">
                                    ...
                                    <div className="publicpoll-group">
                                        <form action='' onSubmit={this.handleSubmitClick} className={`publicpoll-form ${selected ? "publicpoll-form2" : ""}`}>
                                            {polloftheday.options.map((data, key) => {
                                                return (
                                                    <div key={key} className="form-group inputform">
                                                        <div className="custom-control custom-radio">
                                                            <input type="radio" className="custom-control-input" onSelect={this.handleSelect} id={`customControlValidation${key}`} name="radio-stacked" required onChange={this.handleSelect} value={data.id} />
                                                            <label className="custom-control-label" htmlFor={`customControlValidation${key}`}>{data.question}</label>
                                                        </div>
                                                    </div>
                                                )
                                            })}
